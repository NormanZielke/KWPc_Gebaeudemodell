...
def run_peak_test(
    files,
    output_dir,
    peak_from_filename=None,
    time_col=DEFAULT_TIME_COL,
    heat_col=DEFAULT_HEAT_COL,
    show_plot=True,
):
    """
    Vergleicht mehrere nPro-Lastprofile nach Peak-Normierung.
    ...
    peak_from_filename : dict | None
        Optional: Mapping Anzeigename -> Peak laut Dateiname [kW].
        Dient nur zur Kontrolle und beeinflusst die Normierung nicht.
    time_col : str
        Name der Zeitspalte.
    heat_col : str
    ...
    print("\n" + "=" * 80)
    print("nPro-Test: Peak-Normierung")
    print("=" * 80)
    ...
    for name, file_path in files.items():
        ...
        if actual_peak_kw <= 0:
            raise ValueError(
                f"Ungültige Peak-Last in {Path(file_path).name}: "
                f"{actual_peak_kw}"
            )

        df["heat_normalized"] = (
            df[heat_col] / actual_peak_kw
        )
        ...
        print(f"  tatsächlicher Peak: {actual_peak_kw:.6f} kW")

        if peak_from_filename and name in peak_from_filename:
            filename_peak = peak_from_filename[name]
            difference = actual_peak_kw - filename_peak
            profiles[name]["filename_peak_kw"] = filename_peak

            print(f"  Peak laut Dateiname: {filename_peak:.6f} kW")
            print(f"  Differenz: {difference:.6f} kW")
    ...
    comparison_file = (
        output_dir
        / "Vergleich_normierte_Lastprofile_Peak.csv"
    )
    ...
    normalized_profiles_file = (
        output_dir
        / "Normierte_Lastprofile_Peak.csv"
    )
    ...
    fig.update_layout(
        title=(
            "Vergleich Peak-normierter nPro-Wärmelastprofile"
        ),
        xaxis_title="Zeit",
    ...
    interactive_file = (
        output_dir
        / "Vergleich_normierte_Lastprofile_Peak_interaktiv.html"
    )
    fig.write_html(
        interactive_file
    )
...
def _compare_profiles(profiles, column):
    ...
    if not comparison_df.empty:
        ...
        if comparison_df["Identisch"].all():
            print(
                "ERGEBNIS: Alle Peak-normierten Profile "
                "sind innerhalb der Toleranz identisch."
            )
        else:
            print(
                "ERGEBNIS: Die Peak-normierten Profile "
